...
## Как сбросить ключи на 0

Нажми маленькую кнопку в правом нижнем углу и затем кнопку `0` в панели добавления. Это сбросит счетчик и progress-bar на текущем устройстве.

Если нужно сбросить вручную через консоль браузера:

```js
localStorage.removeItem("emetarKeys");
location.reload();
```
